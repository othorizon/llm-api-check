...
- **SEO**: every route is prerendered to static HTML in English (`/`, the default) and Chinese (`/zh`), with `hreflang` (`x-default` → English), Open Graph, JSON-LD and a sitemap. A remembered language choice (header toggle or banner) redirects before first paint via the inline script in `index.html`; without one, Chinese-language browsers opening an English URL are sent to `/zh` the same way, while English-language browsers on `/zh` only get a one-time switch banner. Crawlers render as en-US with no storage, so they never see a redirect. Legacy `/en/*` URLs 301 to the root via `_redirects`.
